...
- Do not attach `[fact]` to meta-descriptions about the model (e.g., "the action distribution is rule-based"). These belong in Limitations, unlabelled.
- Speculative language (`may`, `might`, `could`, `possibly`) must use `[analysis]` or `[hypothesis]`, never `[fact]`.
