...
_CODE_GENERATED_SECTIONS = frozenset({
    "what_to_watch", "hypothesis_review", "risk_review", "next_steps",
})

# Default section order. Includes both LLM-generated and code-generated sections.
# Code-generated sections (what_to_watch, hypothesis_review, next_steps) are
# appended by _append_code_sections() when their content is available.
...
def _append_code_sections(
    ...
    order = section_order or _DEFAULT_SECTION_ORDER
    # Append code-generated sections in order: Risk Review -> What to Watch
    # -> Hypothesis Review -> Next Steps. Risk comes first so the reader
    # sees reliability caveats before observation points and next actions.
    if "risk_review" in order and report_input.risk_audit_md:
        report = report.rstrip() + "\n\n" + report_input.risk_audit_md
    if "what_to_watch" in order and report_input.validation_signals_md:
        report = report.rstrip() + "\n\n" + report_input.validation_signals_md
    if "hypothesis_review" in order and report_input.hypothesis_critiques_md:
        report = report.rstrip() + "\n\n" + report_input.hypothesis_critiques_md
    if "next_steps" in order and report_input.scenario_suggestions_md:
